...
func (this *ip2region) MemorySearch(ipStr string) (*assert.IPv4Info, error) {
	var err error
	...
	i := assert.NewIPv4Info(getLong(this.dbBinStr, dataPtr), this.dbBinStr[(dataPtr)+4:dataPtr+dataLen])
	return i, nil

}

func (this *ip2region) BinarySearch(ipStr string) (i *assert.IPv4Info, err error) {
	if this.totalBlocks == 0 {
	...
	i = assert.NewIPv4Info(getLong(data, 0), data[4:dataLen])
	err = nil
	return
}

func (this *ip2region) BtreeSearch(ipStr string) (i *assert.IPv4Info, err error) {
	ip, err := Ip2long(ipStr)
	...
	data := make([]byte, dataLen)
	this.dbFileHandler.Read(data)
	i = assert.NewIPv4Info(getLong(data, 0), data[4:])
	return
}
...
var (
	invalidIP = errors.New("ip format error")
)
